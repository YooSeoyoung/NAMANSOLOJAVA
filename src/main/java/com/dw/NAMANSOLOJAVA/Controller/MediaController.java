...
public class MediaController {
    ...
    public ResponseEntity<MediaDTO> saveSingleMedia(@RequestBody MediaDTO mediaDTO) {
        return new ResponseEntity<>(mediaService.saveSingleMedia(mediaDTO), HttpStatus.CREATED);
    }

    @PostMapping("/save/multiple")
    public ResponseEntity<List<MediaDTO>> saveMultipleMedia(@RequestBody List<MediaDTO> mediaDTOs) {
        return new ResponseEntity<>(mediaService.saveMultipleMedia(mediaDTOs), HttpStatus.CREATED);
    }

    @GetMapping("/multiple")
    public ResponseEntity<List<MediaDTO>> getMultipleMedia() {
        return new ResponseEntity<>(mediaService.getMultipleMedia(), HttpStatus.OK);
    }

    @DeleteMapping("/delete/{id}")
    public ResponseEntity<MediaDTO> deleteSingleMediaById(@PathVariable Long id) {
